from preprocess import User, Password, Password_Segment
...
with open(args.input, "rb") as password_dump:
    for line in tqdm(get_ascii(password_dump), desc="Encoding", total=count):
        ...
        try:
            ...
            try:
                if not userpw[0] or not userpw[1] or len(userpw[0]) < 6:
                    continue
            except IndexError:
                continue
            current = User(userpw[0], userpw[1], args.norder)
            lab_builder = []
            feat_builder = []
            for lab, feat in current.get_array():
                lab_builder.append(lab)
                feat_builder.append(feat)
            features[current.username_string[:6]] = feat_builder
            labels[current.username_string[:6]] = lab_builder
        except TypeError:
            continue
